...
4. If `key_source` is `"byok"`, the JWT will contain a `byok_key_id` claim. Aegis will fetch this specific user's encrypted API key from KMS.
5. In both cases, the real API key is fetched dynamically just before the proxy request and zeroed immediately after.
...
- **Cost Control**: BYOK users can bypass developer rate limits and budgets without affecting the developer's quota.
